...
class SIM800L:
    def __init__(self):
        self.uart = UART(1, baudrate=9600, tx=config.SIM800_RX, rx=config.SIM800_TX, timeout=2000)
        self.flush()
    ...
    def send_json(self, data, retries=3):
        ...
        for intento in range(1, retries + 1):
            ...
            try:
                ...
                self.send_cmd(f'AT+SAPBR=3,1,"APN","{config.APN}"', 2)
                self.send_cmd("AT+SAPBR=1,1", 8)
                self.send_cmd("AT+SAPBR=2,1", 3)

                # HTTP POST
                self.send_cmd("AT+HTTPINIT", 2)
                self.send_cmd(f'AT+HTTPPARA="URL","{config.SERVER_URL}"', 2)
