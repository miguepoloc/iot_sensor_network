...
from cwt_soil import CwtSoil
from hd38 import HD38
from http_client import get_remote_time, send_data
from machine import I2C, Pin, deepsleep
from power_control import power_off_all, power_on_all
from retry_queue import enqueue, init_queue
from rtc_ds1307 import RtcDs1307
from sd_utils import copy_json, save_json
...
while time.time() - start < 300:  # 5 min
    ...
    for intento in range(3):
        ...
        if all(
            k in cwt_data and cwt_data[k] is not None
            for k in [
                "humidity",
                "nitrogen",
                "phosphorus",
    ...
    if not soil_cwt:
        ...
        soil_cwt = dict.fromkeys(
            ["humidity", "nitrogen", "phosphorus", "potassium", "conductivity", "pH", "salinity", "temperature", "tds"]
        )
